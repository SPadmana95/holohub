...
// ─────────────────────────────────────────────────────────────────────────────
class AdiTofSubscriberOp
    : public holoscan::ros2::ops::SubscriberOp<sensor_msgs::msg::Image> {
 public:
  HOLOSCAN_OPERATOR_FORWARD_ARGS_SUPER(
      AdiTofSubscriberOp,
  ...
  void setup(holoscan::OperatorSpec& spec) override {
    ...
    spec.param(tensor_name_, "tensor_name", "Tensor name",
               "Name used in the output GXF entity", std::string(""));
  ...
  void compute(holoscan::InputContext& /*op_input*/,
    ...
    while (rclcpp::ok()) {
      ...
      if (status == std::future_status::ready) {
        try {
          ...
          HOLOSCAN_LOG_WARN("AdiTofSubscriberOp [{}]: receive() threw: {}",
                            tensor_name_.get(), e.what());
          return;
        }
      }
    ...
    auto alloc_handle = nvidia::gxf::Handle<nvidia::gxf::Allocator>::Create(
    ...
    auto out_entity = nvidia::gxf::CreateTensorMap(
        context.context(), alloc_handle.value(),
    ...
    auto maybe_tensor = out_entity.value().get<nvidia::gxf::Tensor>(
        tensor_name_.get().c_str());
    if (!maybe_tensor) {
    ...
    cudaMemcpy(maybe_tensor.value()->pointer(),
               message.data.data(), n_bytes, cudaMemcpyHostToDevice);
 ...
 public:
  ...
  void compose() override {
    auto bridge = make_resource<holoscan::ros2::Bridge>(
        "aditof_subscriber_resource", "aditof_subscriber_node");
    ...
    auto ab_sub = make_operator<AdiTofSubscriberOp>(
        ...
        holoscan::Arg("qos", holoscan::ros2::QoS(10)),
        holoscan::Arg("pool", pool),
        holoscan::Arg("tensor_name", std::string("ActiveBrightness")));  // matches HolovizOp InputSpec name
...
int main(int argc, char** argv) {
  ...
  while (true) {
    ...
    if (c == 0) {
      ...
      if (name == "fullscreen") fullscreen = true;
    } else if (c == 'h') {
      std::cout
          << "Usage: holoscan_ros2_aditof_subscriber [options]\n"
