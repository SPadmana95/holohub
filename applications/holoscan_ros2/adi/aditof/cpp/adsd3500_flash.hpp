...
class Adsd3500 {
 public:
  bool adsd3500_flash(const std::vector<uint8_t>& file_data,
                      std::shared_ptr<hololink::sensors::Adcam> adcam, bool force = false);
 ...
 private:
  ...
  bool updateAdsd3500MasterFirmware(uint8_t* fw_data, uint32_t fw_len, bool force,
                                    uint32_t expected_crc);
  bool updateAdsd3500SlaveFirmware(uint8_t* fw_data, uint32_t fw_len, bool force,
                                   uint32_t expected_crc);
