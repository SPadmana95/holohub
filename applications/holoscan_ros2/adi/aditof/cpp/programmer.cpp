...
size_t WriteCallback(void* contents, size_t size, size_t nmemb, std::vector<uint8_t>* userp) {
  ...
  userp->insert(
      userp->end(), static_cast<uint8_t*>(contents), static_cast<uint8_t*>(contents) + total_size);
  return total_size;
}
...
std::vector<uint8_t> Programmer::fetch_content(const std::string& content_name) {
  ...
  if (content_metadata["url"]) {
    ...
    throw std::runtime_error(
        fmt::format("No instructions for where to find {} are provided.", content_name));
  }
  ...
  if (actual_size != expected_size) {
    throw std::runtime_error(fmt::format(
        "{} expected_size={} actual_size={}; aborted.", content_name, expected_size, actual_size));
  }
  ...
  if (to_lower(actual_md5) != to_lower(expected_md5)) {
    throw std::runtime_error(fmt::format(
        "{} expected_md5={} actual_md5={}; aborted.", content_name, expected_md5, actual_md5));
  }
...
void Programmer::check_eula() {
  ...
  for (const auto& license : licenses) {
    ...
    if (answer.empty() || (answer[0] != 'y' && answer[0] != 'Y')) {
      throw std::runtime_error(
          "Execution of this script requires an "
