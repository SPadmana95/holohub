...
bool Adsd3500::adsd3500_flash(const std::vector<uint8_t>& file_data,
                              std::shared_ptr<hololink::sensors::Adcam> adcam, bool force) {
  this->force = force;
  adcam_ = adcam;
  ...
  uint32_t master_len = (uint32_t)file_data[8] | ((uint32_t)file_data[9] << 8) |
                        ((uint32_t)file_data[10] << 16) | ((uint32_t)file_data[11] << 24);
  if (master_len == 0 || master_len > ADI_DUAL_FW_SLOT_SIZE - ADI_CHUNK_HEADER_SIZE) {
    std::cerr << "Invalid master firmware size in Slot 0 header: " << master_len << " bytes"
              << std::endl;
    return false;
  }
  ...
  uint32_t slave_len = (uint32_t)file_data[ADI_DUAL_FW_SLOT_SIZE + 8] |
  ...
  if (slave_len == 0 || slave_len > ADI_DUAL_FW_SLOT_SIZE - ADI_CHUNK_HEADER_SIZE) {
    std::cerr << "Invalid slave firmware size in Slot 1 header: " << slave_len << " bytes"
              << std::endl;
    return false;
  }
  ...
  if (master_fw.size() != master_len) {
    std::cerr << "Master firmware buffer size mismatch: expected " << master_len << " bytes, got "
              << master_fw.size() << " bytes" << std::endl;
    return false;
  }
  ...
  std::cout << "[INFO] Header Master CRC : 0x" << std::hex << std::uppercase << std::setw(8)
            << std::setfill('0') << master_expected_crc << std::dec << std::endl;
  ...
  if (slave_fw.size() != slave_len) {
    std::cerr << "Slave firmware buffer size mismatch: expected " << slave_len << " bytes, got "
              << slave_fw.size() << " bytes" << std::endl;
    return false;
  }
  ...
  std::cout << "[INFO] Header Slave CRC  : 0x" << std::hex << std::uppercase << std::setw(8)
            << std::setfill('0') << slave_expected_crc << std::dec << std::endl;
  ...
  snprintf(master_ver_str,
           sizeof(master_ver_str),
           "%d.%d.%d.%d",
           master_fw[0],
           master_fw[1],
           master_fw[2],
           master_fw[3]);
  snprintf(slave_ver_str,
           sizeof(slave_ver_str),
           "%d.%d.%d.%d",
           slave_fw[0],
           slave_fw[1],
           slave_fw[2],
  ...
  if (master_fw[0] != slave_fw[0] || master_fw[1] != slave_fw[1] || master_fw[2] != slave_fw[2] ||
      master_fw[3] != slave_fw[3]) {
    std::cerr << "[ERR] Version mismatch between master (" << master_ver_str << ") and slave ("
              << slave_ver_str << ") firmware payloads. Aborting." << std::endl;
    return false;
  }
  ...
  if (master_found && master_chip_id == ADSD3500_MASTER_CHIP_ID) {
    std::cout << "[INFO] Master Chip ID is: 0x" << std::hex << std::uppercase << std::setw(4)
              << std::setfill('0') << master_chip_id << std::dec << std::endl;
  } else {
    std::cerr << "No ADSD3500 master device detected (chip ID 0x" << std::hex << std::uppercase
  ...
  if (slave_found && slave_chip_id == ADSD3500_SLAVE_CHIP_ID) {
    std::cout << "[INFO] Slave Chip ID is: 0x" << std::hex << std::uppercase << std::setw(4)
              << std::setfill('0') << slave_chip_id << std::dec << std::endl;
  } else {
    slave_found = false;
    ...
    if (dual_query_ok) {
      std::cout << "[INFO] Get Is Dual ADSD3500 Enabled (0x005A): 0x" << std::hex << std::uppercase
                << std::setw(4) << std::setfill('0') << dual_enabled << std::dec << std::endl;
  ...
  if (master_found && slave_found) {
    ...
    std::cout << "Both ADSD3500 devices detected. Updating master and "
                 "slave firmware."
              << std::endl;
    if (!this->updateAdsd3500MasterFirmware(
            master_fw.data(), master_len, force, master_expected_crc)) {
...
bool Adsd3500::updateAdsd3500MasterFirmware(uint8_t* fw_data, uint32_t fw_len, bool force,
  ...
  {
    auto ver = adcam_->get_fw_version_burst_mode(GET_MASTER_FIRMWARE_COMMAND);
    if (ver.size() != 44) {
      std::cerr << "[MASTER] Failed to read current firmware version (got " << ver.size()
                << " bytes, expected 44)" << std::endl;
      return false;
    } else {
      std::memcpy(current_ver, ver.data(), 44);
      std::cout << "[MASTER] Current firmware version  : " << (int)current_ver[0] << "."
                << (int)current_ver[1] << "." << (int)current_ver[2] << "." << (int)current_ver[3]
                << std::endl;
    }
  }
  ...
  if (fw_len >= 4) {
    ...
    snprintf(update_ver_str,
             sizeof(update_ver_str),
             "%d.%d.%d.%d",
             fw_data[0],
             fw_data[1],
             fw_data[2],
             fw_data[3]);
    snprintf(current_ver_str,
             sizeof(current_ver_str),
             "%d.%d.%d.%d",
             current_ver[0],
             current_ver[1],
             current_ver[2],
    ...
    if (below_minimum) {
      std::cerr << "[MASTER] ERROR: Firmware version " << update_ver_str
  ...
  std::cout << "[MASTER] CRC raw result : 0x" << std::hex << std::uppercase << std::setw(8)
            << std::setfill('0') << res.crc_32bit << std::dec << std::endl;
  std::cout << "[MASTER] nResidualCRC   : 0x" << std::hex << std::uppercase << std::setw(8)
            << std::setfill('0') << nResidualCRC << std::dec << std::endl;
  std::cout << "[MASTER] Expected CRC   : 0x" << std::hex << std::uppercase << std::setw(8)
            << std::setfill('0') << expected_crc << std::dec << std::endl;
  if (nResidualCRC != expected_crc) {
    std::cerr << "[MASTER] CRC MISMATCH: computed 0x" << std::hex << std::uppercase << std::setw(8)
              << std::setfill('0') << nResidualCRC << " != expected 0x" << std::setw(8)
              << std::setfill('0') << expected_crc << std::dec << std::endl;
    return false;
  }
...

  {
    ...
    uint16_t reg[9];
    reg[0] = 8;
    for (int i = 0; i < 8; ++i) reg[i + 1] = (uint16_t(p[2 * i]) << 8) | uint16_t(p[2 * i + 1]);
  ...
  for (int i = 0; i < packetsToSend; i++) {
    ...
    if (!status) {
      std::cerr << "[MASTER] Failed to send packet number " << i << " out of " << packetsToSend
                << " packets!" << std::endl;
      return status;
    }

    std::cout << "[MASTER] Packet number: " << i + 1 << " / " << packetsToSend << '\r';
    fflush(stdout);
  }
  std::cout << std::endl;
  std::cout << std::endl;
  ...
  std::cout << "[MASTER] Get status Command 0x" << std::hex << std::uppercase << std::setw(2)
            << std::setfill('0') << Status_Command << std::dec << std::endl;
  ...
  std::cout << "[MASTER] After upgrading new firmware " << std::endl;
  {
    auto ver = adcam_->get_fw_version_burst_mode(GET_MASTER_FIRMWARE_COMMAND);
    if (ver.size() != 44) {
      std::cerr << "[MASTER] Failed to read updated firmware version (got " << ver.size()
                << " bytes, expected 44)" << std::endl;
    } else {
      std::cout << "[MASTER] Updated firmware version   : " << (int)ver[0] << "." << (int)ver[1]
                << "." << (int)ver[2] << "." << (int)ver[3] << std::endl;
    }
  }
  sleep(1);
...
bool Adsd3500::updateAdsd3500SlaveFirmware(uint8_t* fw_data, uint32_t fw_len, bool force,
                                           uint32_t expected_crc) {
  bool status = true;
  uint8_t Wait_Time = 0;
  uint16_t Status_Command = 0;
  ...
  */
  ...
  uint8_t current_ver[44] = {0};
  {
    auto ver = adcam_->get_fw_version_burst_mode(GET_SLAVE_FIRMWARE_COMMAND);
    if (ver.size() != 44) {
      std::cerr << "[SLAVE] Failed to read current firmware version (got " << ver.size()
                << " bytes, expected 44)" << std::endl;
    } else {
      std::memcpy(current_ver, ver.data(), 44);
      std::cout << "[SLAVE] Current firmware version   : " << (int)current_ver[0] << "."
                << (int)current_ver[1] << "." << (int)current_ver[2] << "." << (int)current_ver[3]
                << std::endl;
    }
  }
  ...
  if (fw_len >= 4) {
    ...
    if (below_minimum) {
      std::cerr << "[SLAVE] ERROR: Firmware version " << update_ver_str
                << " is below the minimum required version 8.1.0.0. Aborting." << std::endl;
      if (!adcam_->switch_from_burst_to_standard()) {
  ...
  crc_output_t res = compute_crc(&crc_params, fw_data, fw_len);
  nResidualCRC = ~res.crc_32bit;
  std::cout << "[SLAVE] CRC raw result : 0x" << std::hex << std::uppercase << std::setw(8)
            << std::setfill('0') << res.crc_32bit << std::dec << std::endl;
  std::cout << "[SLAVE] nResidualCRC   : 0x" << std::hex << std::uppercase << std::setw(8)
            << std::setfill('0') << nResidualCRC << std::dec << std::endl;
  std::cout << "[SLAVE] Expected CRC   : 0x" << std::hex << std::uppercase << std::setw(8)
            << std::setfill('0') << expected_crc << std::dec << std::endl;
  if (nResidualCRC != expected_crc) {
    std::cerr << "[SLAVE] CRC MISMATCH: computed 0x" << std::hex << std::uppercase << std::setw(8)
              << std::setfill('0') << nResidualCRC << " != expected 0x" << std::setw(8)
              << std::setfill('0') << expected_crc << std::dec << std::endl;
    return false;
  }
  ...
  for (int i = 0; i < packetsToSend; i++) {
    ...
    if (!status) {
      std::cerr << "[SLAVE] Failed to send packet number " << i << " out of " << packetsToSend
                << " packets!" << std::endl;
      return status;
    }

    std::cout << "[SLAVE] Packet number: " << i + 1 << " / " << packetsToSend << '\r';
    fflush(stdout);
  }
  std::cout << std::endl;
  std::cout << std::endl;
  ...
  status = read_cmd(GET_IMAGER_STATUS_CMD, &Status_Command);
  std::cout << "[SLAVE] Get status Command 0x" << std::hex << std::uppercase << std::setw(2)
            << std::setfill('0') << Status_Command << std::dec << std::endl;
  ...
  */
  ...
  std::cout << "[SLAVE] After upgrading new firmware " << std::endl;
  {
    auto ver = adcam_->get_fw_version_burst_mode(GET_SLAVE_FIRMWARE_COMMAND);
    if (ver.size() != 44) {
      std::cerr << "[SLAVE] Failed to read updated firmware version (got " << ver.size()
                << " bytes, expected 44)" << std::endl;
    } else {
      std::cout << "[SLAVE] Updated firmware version    : " << (int)ver[0] << "." << (int)ver[1]
                << "." << (int)ver[2] << "." << (int)ver[3] << std::endl;
    }
  }
  sleep(1);
...
bool Adsd3500::write_payload(uint8_t* payload, uint16_t payload_len) {
  if (payload_len == 0 || payload_len % 2 != 0) {
    std::cerr << "write_payload: payload_len must be a non-zero even number, got " << payload_len
              << std::endl;
    return false;
  }
  ...
  for (uint16_t i = 0; i < word_count; i++) {
    reg[i + 1] = (uint16_t(payload[2 * i]) << 8) | uint16_t(payload[2 * i + 1]);
  }
