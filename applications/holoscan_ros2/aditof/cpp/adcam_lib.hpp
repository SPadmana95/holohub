...
//
// Column layout:
//   { mode_number, mipi_w, mipi_h,  px_w,  px_h,  phase_depth_bits, ab_bits, confidence_bits,
...
constexpr AdcamModeConfig adtf3066_standardModes[] = {
    ...
    /* Mode 0 */ {0, 2560, 640, 512, 640, 6, 6, 2, 1, 1, 2},  // VGA   depth+AB+conf, AB avg on
                                                              /* Mode 1 */
    {1, 2560, 640, 512, 640, 6, 6, 2, 1, 1, 2},               // VGA   depth+AB+conf, AB avg on
                                                              /* Mode 7 */
    {7, 2560, 640, 512, 640, 6, 6, 2, 1, 1, 2},               // VGA   depth+AB+conf, AB avg on

    // ---- QVGA modes (256×320, 1 Gbps MIPI) ----
    /* Mode 3 */
    {3, 1280, 320, 256, 320, 6, 6, 2, 1, 1, 2},  // QVGA  depth+AB+conf, AB avg on
                                                 /* Mode 6 */
    {6, 1280, 320, 256, 320, 6, 6, 2, 1, 1, 2},  // QVGA  depth+AB+conf, AB avg on
                                                 /* Mode 8 */
    {8, 1280, 320, 256, 320, 6, 6, 2, 1, 1, 2},  // QVGA  depth+AB+conf, AB avg on
};
...
class ADII2CExpander {
 public:
  ADII2CExpander(std::shared_ptr<hololink::Hololink> hololink_, uint32_t i2c_bus,
                 uint32_t expander_addr);

  void configure(I2CExpanderOutputEN output_en = I2CExpanderOutputEN::DEFAULT);

  void set_register(uint16_t register_, uint32_t value,
...
class ADIGPIOCtl {
 public:
  ADIGPIOCtl(std::shared_ptr<hololink::Hololink> hololink_, hololink::Metadata& channel_metadata,
             uint32_t pin);
...
class Adcam {
 public:
  ...
  std::vector<uint8_t> set_register16_response(std::shared_ptr<hololink::sensors::Adcam> adcam_inst,
                                               uint16_t* register_blob, size_t resp_len);
 ...
 private:
  ...
  void i2c_write_read(std::shared_ptr<hololink::sensors::Adcam> adcam_inst,
                      const std::vector<uint8_t>& write_bytes, size_t read_byte_count,
                      std::vector<uint8_t>* out_reply);
