...
static void log_reply_prefix(const char* label, const std::vector<uint8_t>& reply) {
  if (reply.size() >= 2) {
    HOLOSCAN_LOG_INFO("{} bytes={} {:02X} {:02X}",
                      label,
                      reply.size(),
...
ADII2CExpander::ADII2CExpander(std::shared_ptr<hololink::Hololink> hololink_, uint32_t i2c_bus,
...
void ADII2CExpander::set_register(uint16_t register_, uint32_t value,
  ...
  HOLOSCAN_LOG_DEBUG("ADCAM Expander Config set addr=0x{:X} value=0x{:X}", expander_addr_, value);
...
ADIGPIOCtl::ADIGPIOCtl(std::shared_ptr<hololink::Hololink> hololink_,
                       hololink::Metadata& channel_metadata, uint32_t pin)
    : reset_pin_(pin) {
...
bool ADIGPIOCtl::wait_for_low_and_set_high_profile(uint32_t pin) {
  ...
  while (elapsed_us < timeout_us) {
    ...
    if (value == gpio_->HIGH) {
      ...
      HOLOSCAN_LOG_INFO("GPIO {} observed HIGH after {} us ({} loops). Restoring LOW.",
                        pin,
                        total_us,
                        loop_count);
  ...
  HOLOSCAN_LOG_WARN("Timeout: GPIO {} did not reach target state within {} us ({} loops)",
                    pin,
                    timeout_us,
                    loop_count);
...
Adcam::Adcam(std::shared_ptr<hololink::DataChannel> hololink_channel,
             uint32_t hololink_i2c_controller_address, hololink::Metadata& channel_metadata,
             uint32_t adcam_mode, uint32_t reset_pin)
    : hololink_(hololink_channel->hololink()),
      ...
      expander0_(hololink_, hololink::CAM_I2C_BUS, EXPANDER_0_I2C_BUS_ADDRESS),
      expander1_(hololink_, hololink::CAM_I2C_BUS, EXPANDER_1_I2C_BUS_ADDRESS),
  ...
  if (!init_cfg) {
    init_cfg = adcam_find_mode(adtf3066_standardModes, static_cast<int>(adcam_mode));
  }
  if (!init_cfg) {
    throw std::runtime_error(fmt::format("Adcam: unsupported adcam_mode {}", adcam_mode));
  }
...
void Adcam::i2c_write_read(const std::vector<uint8_t>& write_bytes, size_t read_byte_count,
  ...
  auto reply = i2c_->i2c_transaction(ADCAM_I2C_BUS_ADDRESS, write_bytes, read_byte_count, timeout);
...
void Adcam::set_mipi(std::shared_ptr<hololink::sensors::Adcam> adcam_inst) {
  ...
  auto resp = adcam_inst->set_register16_response(reg0, 2);
...
void Adcam::set_mode() {
  ...
  const char* imager_str = (imager_enum == AdcamImagerType::ADSD3100)   ? "ADSD3100"
                           : (imager_enum == AdcamImagerType::ADTF3066) ? "ADTF3066"
                                                                        : "Unknown";
  const AdcamModeConfig* cfg = nullptr;
  switch (imager_enum) {
    case AdcamImagerType::ADSD3100:
      cfg = adcam_find_mode(adsd3100_standardModes, static_cast<int>(adcam_mode_));
      break;
    case AdcamImagerType::ADTF3066:
      cfg = adcam_find_mode(adtf3066_standardModes, static_cast<int>(adcam_mode_));
      break;
    default:
      throw std::runtime_error(
          fmt::format("set_mode: unsupported imager type (raw={})", imager_type_));
  }
  if (!cfg) {
    throw std::runtime_error(fmt::format("set_mode: mode {} not found for imager {} (raw={})",
                                         adcam_mode_,
                                         imager_str,
                                         imager_type_));
  }
  ...
  HOLOSCAN_LOG_INFO(
      "Setting imager mode={} reg=0x{:04X} settings=0x{:04X}", adcam_mode_, mode_reg, mode_setting);
...
void Adcam::read_nvm_config() {
  ...
  holoscan::log_warn(
      "read_nvm_config: register blobs >64-bit need a "
      "byte-array API in C++ if extended further.");
...
void Adcam::get_imager_type_and_ccb_version() {
  ...
  if (resp.size() < 2) {
    HOLOSCAN_LOG_ERROR(
        "get_imager_type_and_ccb_version: incomplete "
  ...
  switch (imager_enum) {
    case AdcamImagerType::ADSD3100:
      ...
      mode_cfg = adcam_find_mode(adsd3100_standardModes, static_cast<int>(adcam_mode_));
      break;
    case AdcamImagerType::ADTF3066:
      // ADTF3066: VGA and QVGA modes in adtf3066_standardModes.
      mode_cfg = adcam_find_mode(adtf3066_standardModes, static_cast<int>(adcam_mode_));
      break;
    default:
      HOLOSCAN_LOG_ERROR("get_imager_type_and_ccb_version: unsupported imager type (raw={})",
                         imager_type_);
      throw std::runtime_error(fmt::format("Unsupported imager type (raw={})", imager_type_));
  }
...
bool Adcam::switch_from_standard_to_burst() {
  uint16_t reg[] = {2, SET_SWITCH_TO_BURST_MODE, 0x0000};
  if (!set_register16_no_response(reg)) {
    HOLOSCAN_LOG_ERROR("switch_from_standard_to_burst: failed to switch to burst mode");
    return false;
  }
  return true;
}
...
bool Adcam::switch_from_burst_to_standard() {
  uint16_t reg[] = {8, 0xAD00, 0x0010, 0x0000, 0x0000, 0x1000, 0x0000, 0x0100, 0x0000};
  if (!set_register16_no_response(reg)) {
    HOLOSCAN_LOG_ERROR("switch_from_burst_to_standard: failed to switch to standard mode");
    return false;
  }
  return true;
}
...
std::vector<uint8_t> Adcam::get_fw_version(uint8_t cmd) {
  ...
  uint16_t reg3[] = {
      8, 0xAD00, 0x2C05, 0x0000, 0x0000, 0x3100, 0x0000, static_cast<uint16_t>(cmd << 8), 0x0000};
...
std::vector<uint8_t> Adcam::get_fw_version_burst_mode(uint8_t cmd) {
  ...
  if (resp.size() < 44) {
    HOLOSCAN_LOG_ERROR(
        "get_fw_version_burst_mode: incomplete response "
        "bytes={} (expected 44)",
        resp.size());
  }
  return resp;
...
bool Adcam::get_ChipID(uint16_t cmd) {
  ...
  if (resp.size() >= 2) {
    HOLOSCAN_LOG_INFO("Chip ID bytes={} ID {:02X} {:02X}",
                      resp.size(),
...
void Adcam::configure_converter(std::shared_ptr<hololink::csi::CsiConverter> converter) {
  ...
  converter->configure(start_byte, received_line_bytes, width_, height_, csi::PixelFormat::RAW_8);
}
