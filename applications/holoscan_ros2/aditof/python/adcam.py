...
AdcamCcbMode = namedtuple(
    "AdcamCcbMode",
    [
        "user_defined_mode", "cfg_mode", "height", "width", "n_freq", "p0_mode",
        "temp_mode", "ini_index", "default_mode", "passive_mode_flag", "n_phases",
        "n_captures", "spare4", "spare5", "spare6", "spare7", "spare8",
    ],
)
...
class adcam:
    ...
    def read_modes_from_ccb(self):
        ...
        register = struct.pack(
            ">8H", 0xAD00 | length_high, (length_low << 8) | READ_PAYLOAD_MODE_MAP_CMD,
            0, 0, checksum << 8, 0, 0, 0,
        )
        try:
            reply = self._i2c.i2c_transaction(
                self.ADCAM_I2C_BUS_ADDRESS, register, READ_PAYLOAD_MODE_MAP_LEN,
                timeout=hololink_module.Timeout(30, retry_s=0.2),
            )
        except Exception as error:
            logging.error(f"read_modes_from_ccb: mode map read failed: {error}")
            reply = None
        finally:
        ...
        for index in range(ADCAM_CCB_MODE_COUNT):
            mode = AdcamCcbMode(*struct.unpack_from("<BBHHBBBBBBBBHHHHH", reply, index * 24))
