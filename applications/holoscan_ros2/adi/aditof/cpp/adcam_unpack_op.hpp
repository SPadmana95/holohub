...
void shift_and_cast_kernel(const uint16_t* in, uint8_t* out, int count, cudaStream_t stream);

void grayscale_kernel_launch(const uint16_t* input, uint8_t* rgb, int size, cudaStream_t stream,
                             float max_val);

void jet_kernel_launch(const uint16_t* depth, uint8_t* rgb, int size, cudaStream_t stream);

void unpack_kernel_launch(const uint8_t* raw, uint16_t* depth, uint16_t* conf, uint16_t* ab,
